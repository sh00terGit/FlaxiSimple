...
//    }
    ...
    public function connect() {
        ...
        $dsn = sprintf(
                'mysql:dbname=%s;host=%s;port=%d;charset=%s', $this->dbname, $this->host, $this->port, $this->charset
        );
        try {
    ...
    function select($table, $conditions = array(), $columns = array(), $sort = array(), $limit = 0, $offset = 0) {
        ...
        $sql = sprintf(
                'SELECT %s FROM %s%s%s%s;', ($select == '*') ? '*' : implode(', ', $select), $this->quoteIdentifier($table), $where ? ' WHERE ' . implode(' AND ', $where) : '', $order_by ? ' ORDER BY ' . implode(', ', $order_by) : '', $limit ? sprintf(' LIMIT %d, %d', $offset, $limit) : ''
        );
        $this->query($sql, $bind);
        return $this;
    }
    ...
    public function update($table, $data, $conditions) {
        ...
        $sql = sprintf(
                'UPDATE %s SET %s WHERE %s;', $this->quoteIdentifier($table), implode(', ', $set), implode(' AND ', $where)
        );
    ...
    public function delete($table, $conditions) {
        ...
        $sql = sprintf(
                'DELETE FROM %s WHERE %s;', $this->quoteIdentifier($table), implode(' AND ', $where)
        );
        return $this->query($sql, $bind)->affectedRows();
    }
